...
fun QuizScreen(
    ...
    val totalQuestions by quizViewModel.totalQuestions.collectAsState()
    ...
    val remainingTime by quizViewModel.remainingTime.collectAsState()
    val coolDownTime by quizViewModel.coolDownTime.collectAsState()

    LaunchedEffect(Unit) {
        quizViewModel.updateQuestion(startedBefore)
    }
    ...
    if (quizViewModel.isQuizFinished.collectAsState(initial = false).value) {
        ...
        if (totalQuestions == 0 && errorMessage.isEmpty()) {
                ...
                currentQuestion?.countries?.forEach { country ->
                    ...
                    Row(
                        modifier = Modifier
                            ...
                            .clickable {
                                if (remainingTime != 0) {
                                    quizViewModel.registerAnswer(country.id)
                                }
                            }
                            .background(
                                bgColor,
